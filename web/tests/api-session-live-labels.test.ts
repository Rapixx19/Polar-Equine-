...
describe("POST /api/sessions/[id]/live-labels", () => {
  ...
  it("400 on invalid label", async () => {
    getUserMock.mockResolvedValueOnce({ id: USER_ID });
    const { POST } = await import("@/app/api/sessions/[id]/live-labels/route");
    const res = await POST(fakeReq({ t_ms: 1000, label: "gallop" }), {
      params: Promise.resolve({ id: SESSION_ID }),
    });
    expect(res.status).toBe(400);
  });
  ...
  it("201 success — row inserted with caller's rider_id", async () => {
    getUserMock.mockResolvedValueOnce({ id: USER_ID });
...
describe("GET /api/sessions/[id]/live-labels", () => {
  ...
  it("200 returns labels ordered by t_ms", async () => {
    ...
    selectMock.mockResolvedValueOnce({
      data: [
        { id: "a", t_ms: 1000, label: "walk", created_at: "2026-05-15T08:00:00Z" },
        { id: "b", t_ms: 5000, label: "trot", created_at: "2026-05-15T08:00:05Z" },
        { id: "c", t_ms: 9000, label: "canter", created_at: "2026-05-15T08:00:09Z" },
      ],
      error: null,
    });
    const { GET } = await import("@/app/api/sessions/[id]/live-labels/route");
    ...
    expect(body.labels.map((l) => l.label)).toEqual(["walk", "trot", "canter"]);
